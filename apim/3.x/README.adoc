...
=== 1. Install The Gravitee Helm Chart Repo

* Add the official Gravitee.io Helm chart repo: 
+
....
...
=== 2. Create The Configuration File & Set Custom Values 
Before you continue installing the Helm chart, create a `values.yml` configuration file and set any applicable custom values in it. 
...
# (remove '/gateway' prefix from URL before sending request)
gateway:
  ingress:
    path: /gateway/(.*)
    annotations:
...
es:
  endpoints:
    - http://elastic-elasticsearch-data.default.svc.cluster.local:9200
----
...
Use the parameter details below to configure common features such as:
...
____
*Tip*: You can use the default values.yml
____
...
1. The easiest way is to provide the
...
|===
|Parameter |Description |Default
|`+mongo.rsEnabled+` |Whether Mongo replica set is enabled or not
|`+true+`

|`+mongo.rs+` |Mongo replica set name |`+rs0+`
...
|===
|Parameter |Description |Default
|`+mongodb-replicaset.enabled+` |Enable deployment of Mongo replica set
|`+false+`
|===

See
https://github.com/helm/charts/tree/master/stable/mongodb-replicaset[MongoDB
replica set] for detailed documentation on Helm chart.


==== PostgreSQL Database (via JDBC Connection) 
...
NOTE: When you install APIM, it automatically uses the default value for the host and runs on apim.example.com. To change this default value to a specific host name for your APIM installation, you must modify the `gateway.ingress.hosts` parameter (listed in the link:/apim/3.x/apim_installguide_kubernetes.html#gravitee_gateway[Gravitee Gateway table] below) in the `values.yml` configuration file. 

Check that PostgreSQL pod is up and running before proceeding. 
...
=== 4. Install The Gravitee Helm Chart
...
*Note:* If you're using Helm 3, this name parameter is no longer valid. Instead, please 
https://helm.sh/docs/faq/#release-names-are-now-scoped-to-the-namespace[check here] for details. 
