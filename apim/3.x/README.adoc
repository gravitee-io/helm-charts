...
== Gravitee.io API Management 3.x Helm Chart
...
[source,bash]
----
$ helm install --name graviteeio-apim graviteeio/apim3x
----

To install the chart using the chart archive, run:

....
$ helm install apim3x-3.0.0.tgz
....
...
|===
|Parameter |Description |Default
|`+mongo-replicaset.enable+` |Enable deployment of Mongo replicaset
|`+false+`
|===
...
|===
|Parameter |Description |Default
|`+elasticsearch.enable+` |Enable deployment of Elasticsearch cluster
|`+false+`
|===
...
|`+api.env+` |Environment variables, defined as a list of `name` and `value` as specified in
...
|`+gateway.env+` |Environment variables, defined as a list of `name` and `value` as specified in
https://kubernetes.io/docs/tasks/inject-data-application/define-environment-variable-container/[Kubernetes
documentation] |`+null+`
