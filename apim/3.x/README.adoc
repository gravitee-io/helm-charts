...
include the updated config file in this installation: **`$ helm install --name graviteeio-apim3x graviteeio/apim3 -f values.yaml`.**
...
NOTE: When you install APIM, it automatically uses the default value for the host and runs on `apim.example.com`. To use a specific host name, search through our documentation and replace any values in the `values.yml` configuration file that use the default value of `apim.example.com`. 
